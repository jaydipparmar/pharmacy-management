...
<head>
    ...
    <script>
        ...
        function increaseQty(id) {
            const item = cart.find(c => c.medicine.id === id);
            if(item && item.quantity < item.medicine.stockQuantity) {
                item.quantity += 1;
                renderCart();
            } else {
                alert('Not enough stock available');
        ...
        function decreaseQty(id) {
            const idx = cart.findIndex(c => c.medicine.id === id);
            if(idx !== -1) {
                cart[idx].quantity -= 1;
                    ...
                    cartDiv.innerHTML += `
                        <div class="flex-between" style="margin-bottom: 1rem; align-items: center;">
                            <div style="flex: 1;">
                                ...
                                <div class="text-muted" style="font-size: 0.85rem;">₹\${item.price.toFixed(2)} each</div>
                            </div>
                            <div style="display: flex; align-items: center; gap: 0.5rem;">
                                <button class="btn-secondary" style="padding: 0.25rem 0.5rem; font-size: 0.8rem;" onclick="decreaseQty(\${item.medicine.id})">-</button>
                                <span style="min-width: 2rem; text-align: center;">\${item.quantity}</span>
                                <button class="btn-secondary" style="padding: 0.25rem 0.5rem; font-size: 0.8rem;" onclick="increaseQty(\${item.medicine.id})">+</button>
                                <button class="btn-danger" style="padding: 0.25rem 0.5rem; font-size: 0.8rem;" onclick="removeFromCart(\${item.medicine.id})">×</button>
                            </div>
